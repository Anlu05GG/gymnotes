...
export class RutinaService {
  ...
  listarPorUsuario(usuarioId: number): Observable<Rutina[]> {
    return this.http
      .get<Rutina[]>(`${this.apiUrl}`, { params: { usuarioId } as any })
      .pipe(catchError(this.handle));
  }

  crearRutina(nombre: string, usuarioId: number): Observable<Rutina> {
    return this.http
      .post<Rutina>(this.apiUrl, { nombre, usuarioId })
      .pipe(catchError(this.handle));
  }

  borrarRutina(rutinaId: number): Observable<void> {
    return this.http
      .delete<void>(`${this.apiUrl}/${rutinaId}`)
      .pipe(catchError(this.handle));
  }
  ...
  listarItems(rutinaId: number): Observable<RutinaItemDTO[]> {
    return this.http
      .get<RutinaItemDTO[]>(`${this.apiUrl}/${rutinaId}/items`)
      .pipe(catchError(this.handle));
  }
  ...
  addItem(
    ...
    return this.http
      .post<RutinaItemDTO>(`${this.apiUrl}/${rutinaId}/items`, {
        ejercicioId,
        seriesObjetivo,
